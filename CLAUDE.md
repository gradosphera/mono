...
**Базовая ветка mono-ai-1 — `dev`.** Никаких прямых push в `dev` / `main` / `mvp`. **Никогда не работать в working tree ветки `dev`.** При старте задачи — сразу создать feature-ветку (`feat/...` или `fix/...`) от dev и работать в ней. Коммитить туда свободно мелкими коммитами; PR в dev — когда дойдём до конечного результата. Merge делает пользователь на GitHub.

**Исключение:** явная фраза «закоммить и запушь прямо в ветку» / «прямой пуш в `<branch>`». Тогда без PR — только на ту итерацию, без переноса правила на следующее.
