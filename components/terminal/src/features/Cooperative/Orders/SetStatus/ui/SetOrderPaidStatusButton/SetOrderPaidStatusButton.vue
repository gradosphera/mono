<template lang="pug">
q-btn(flat size="sm" @click="showDialog=true" icon="check") отметить оплаченным
    ...
    ModalBase(title='отметить оплаченным')
      Form(:handler-submit="setPaid" :is-submitting="isSubmitting" :button-cancel-txt="'Отменить'" :button-submit-txt="'Продолжить'" @cancel="close").q-pa-sm
        div(style="max-width: 300px;")
          p Вы уверены, что хотите отметить счёт оплаченным? Система обработает платеж сразу после получения отметки - совет кооператива получит пакет документов для голосования.
