...
| **IN_PROGRESS** | В работе — исполнитель (или ответственный) начал делать. **С этого момента идёт почасовое начисление времени** для задач без оценки. |
...
- **Только Мастер (или Совет) переводит задачу в DONE.** Ответственный и исполнитель этого сделать не могут.
- **Отправить задачу из IN_PROGRESS в ON_REVIEW** могут Мастер и Ответственный, но не рядовой исполнитель. Так работает единоначалие: за результат отвечает один человек.
- **Отменить задачу в работе** (IN_PROGRESS → CANCELED) могут Мастер, Исполнитель и Совет — но **не Ответственный**. Если вы взялись как submaster, выйти, отменив задачу, нельзя.

В выпадающем меню статуса (как в строке задачи на доске, так и на её странице) вы видите только те статусы, в которые вы вправе перевести задачу. Если перехода в списке нет — значит, прямо сейчас вам он недоступен.
