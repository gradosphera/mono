...
После `pushrslt` РИД проходит через два **акта приёма-передачи**:
...
- **Акт приёма-передачи РИД** (`signact2`) — подписывает Председатель. Этим кооператив принимает результат интеллектуальной деятельности от пайщика по указанной стоимости. Право требования возврата паевого взноса фиксируется в Кошельке пайщика.

Между подписями — формальный шаг признания Советом: Председатель проводит решение по Компоненту через стандартную цепочку (подтверждение, голосование, авторизация, исполнение) — чтобы приём имущественного паевого взноса был решением кооператива как организации, а не одного Мастера.

Когда оба акта подписаны, у пайщика появляется кнопка **«Получить долю в ОАП»** — дальше он выбирает, как распределить свою долю между Главным кошельком и ЦПП «Благорост» (см. *Результат и получение доли*).
