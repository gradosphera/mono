...
import { HttpApiError } from '../../../src/utils/httpApiError';
...
describe('Error middlewares', () => {
  describe('Error converter', () => {
    test('should return the same ApiError object it was called with', () => {
      const error = new HttpApiError(httpStatus.BAD_REQUEST, 'Any error');
      const next = jest.fn();
    ...
    test('should convert an Error to ApiError and preserve its status and message', () => {
      ...
      expect(next).toHaveBeenCalledWith(expect.any(HttpApiError));
  ...
  describe('Error handler', () => {
    ...
    test('should send internal server error status and message if in production mode and error is not operational', () => {
      ...
      const error = new HttpApiError(httpStatus.BAD_REQUEST, 'Any error', false);
      const res = httpMocks.createResponse();
      const sendSpy = jest.spyOn(res, 'send');
