...
describe.skip('Бухгалтерский баланс НКО (BuhotchGenerator)', () => {
  ...
  it('считает балансы из ledger в тыс. руб. (текущий отчётный)', () => {
    const input = {
      ...baseInput,
      reportType: ReportType.BUHOTCH as const,
      ledgerData: [
        { accountId: 51000, name: 'Р/С', balanceCurrent: 150000, balancePrevious: 0, balancePrePrevious: 0 },
        { accountId: 80000, name: 'Пай', balanceCurrent: 50000, balancePrevious: 0, balancePrePrevious: 0 },
        { accountId: 86000, name: 'ЦФ', balanceCurrent: 100000, balancePrevious: 0, balancePrePrevious: 0 },
      ],
    };
    const result = gen.generate(input);
    // ДенежнСр = счёт 51: 150000 ÷ 1000 = 150 тыс.
    expect(result.xml).toMatch(/<ДенежнСр[^>]*СумОтч="150"/);
    // ЦелевСредства = счёт 80 (50) + счёт 86 (100) = 150 тыс.
    expect(result.xml).toMatch(/<ЦелевСредства[^>]*СумОтч="150"/);
    // Актив = ДенежнСр (80/86 уходят в пассив) = 150 тыс.
    expect(result.xml).toMatch(/<Актив[^>]*СумОтч="150"/);
    expect(result.xml).toMatch(/<Пассив[^>]*СумОтч="150"/);
  });

  it('прокидывает corrections в СумПрдщ/СумПрдшв для <Актив>/<Пассив>', () => {
    const input = {
      ...baseInput,
      reportType: ReportType.BUHOTCH as const,
      corrections: [
        { accountDisplayId: '51', balancePrevious: 120000, balancePrePrevious: 80000 },
        { accountDisplayId: '86', balancePrevious: 100000, balancePrePrevious: 60000 },
      ],
    };
    const result = gen.generate(input);
    // Актив (51 корректировка) — 120 тыс. прдщ, 80 тыс. прдшв.
    expect(result.xml).toMatch(/<Актив[^>]*СумПрдщ="120"[^>]*СумПрдшв="80"/);
    // Пассив (86 корректировка) — 100 тыс. прдщ, 60 тыс. прдшв.
    expect(result.xml).toMatch(/<Пассив[^>]*СумПрдщ="100"[^>]*СумПрдшв="60"/);
  });

  it('имя файла NO_BOUPR_<tax>_<tax>_<inn><kpp>_<date>_<uuid>', () => {
    const result = gen.generate({ ...baseInput, reportType: ReportType.BUHOTCH });
    expect(result.fileName).toMatch(
      /^NO_BOUPR_7701_7701_7701234567770101001_\d{8}_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/,
    );
  });
...
describe('РСВ (RsvGenerator)', () => {
  ...
  it('<СвПред> представителя несёт НаимОрг (особенность РСВ)', () => {
    ...
    expect(result.xml).toMatch(/<СвПред[^>]*НаимОрг="[^"]+"/);
  });
...
describe('ЕФС-1 (Fss4Generator, СФР)', () => {
  ...
  it('имя файла СФР_<рег>_ЕФС-1_…', () => {
    const result = gen.generate({ ...baseInput, reportType: ReportType.FSS4, period: 1 });
    expect(result.fileName).toMatch(
      /^СФР_7701234567_ЕФС-1_\d{8}_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/,
    );
  });
...
describe('Уведомление по УСН (UusnGenerator)', () => {
  ...
  it('коды кварталов (21/31/33/34) + НомерМесКварт 01..04', () => {
    for (const [q, periodCode] of [[1, '21'], [2, '31'], [3, '33'], [4, '34']] as const) {
      const xml = gen.generate({ ...baseInput, reportType: ReportType.UUSN, period: q }).xml;
  ...
  it('бросает ошибку при period вне 1..4', () => {
    const result = gen.generate({ ...baseInput, reportType: ReportType.UUSN, period: 5 });
    expect(result.isValid).toBe(false);
    expect(result.errors.join('|')).toContain('1 до 4');
  });

  it('имя файла UT_UVISCHSUMNAL_<inn>_<kpp>_<date>_<uuid>', () => {
    const result = gen.generate({ ...baseInput, reportType: ReportType.UUSN, period: 1 });
    expect(result.fileName).toMatch(
      /^UT_UVISCHSUMNAL_7701234567_770101001_\d{8}_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/,
    );
  });
