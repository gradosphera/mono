...
import { IConfig } from '../../coopgram-extension.module';
...
export class CoopgramApplicationService {
  private readonly logger = new Logger(CoopgramApplicationService.name);
    ...
    try {
      ...
      if (email) {
        ...
        if (synapseUser) {
          ...
          await this.addUserToCoopgramRooms(synapseUser.user_id, account);
  ...
  async createMatrixAccount(coopUsername: string, matrixUsername: string, password: string): Promise<boolean> {
    ...
    try {
      ...
      // Добавляем пользователя в комнаты чаткооп
      await this.addUserToCoopgramRooms(registerResponse.user_id, account);
   ...
   */
  private async addUserToCoopgramRooms(matrixUserId: string, account: AccountDomainEntity): Promise<void> {
    try {
      // Получаем конфигурацию чаткооп
      const coopgramConfig = await this.extensionRepository.findByName('coopgram');
      if (!coopgramConfig || !coopgramConfig.config.isInitialized) {
        console.warn('Coopgram extension not initialized, skipping room assignment');
        return;
      }

      const { spaceId, membersRoomId, councilRoomId } = coopgramConfig.config;
