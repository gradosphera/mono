...
Система учёта времени предназначена для автоматического отслеживания рабочего времени вкладчиков в проектах кооператива. Основная идея заключается в том, что вкладчики получают "пассивный доход" времени за активные задачи, которые они ведут в проектах.

### Основные принципы работы

1. **Автоматический учёт**: Система работает по крону каждый час и автоматически начисляет время вкладчикам за активные задачи (IN_PROGRESS)
2. **Индивидуальные лимиты**: Каждый вкладчик имеет индивидуальный лимит часов в день (поле `hours_per_day`), по умолчанию 0 часов
3. **Равномерное распределение**: Если у вкладчика несколько активных задач, общее доступное время (согласно индивидуальному лимиту) распределяется между ними поровну
...
{
  _id: string,
  contributor_hash: string,    // Хеш вкладчика
...
async trackTime(): Promise<void> {
  ...
  // Получаем всех активных вкладчиков из всех кооперативов
  const activeContributors = await this.getAllActiveContributors();

  // Обрабатываем каждого вкладчика
...
private async trackTimeForContributor(contributor, date): Promise<void> {
  // 1. Получаем активные задачи вкладчика
...
- Вкладчик имеет 4 активные задачи
- Каждый час начисляется по 0.25 часа на каждую задачу
- Максимум 2 часа на задачу в день (8/4)

#### Сценарий 4: Вкладчик с индивидуальным лимитом 6 часов
- Вкладчик имеет 2 активные задачи, лимит 6 часов в день
- Каждый час начисляется по 0.5 часа на каждую задачу
...
#### Сценарий 1: Вкладчик работает над задачей, но не завершает её
- Вкладчик имеет активную задачу IN_PROGRESS
- Каждый час начисляется 1 час в pending_hours
- available_hours = 0 (нет завершённых задач)
- total_uncommitted_hours = pending_hours

#### Сценарий 2: Вкладчик завершает задачу
- Вкладчик работает над задачей IN_PROGRESS (начисляется время в pending_hours)
...
- Вкладчик имеет 2 активные задачи IN_PROGRESS (начисляется по 0.5 часа на каждую в pending_hours)
...
## Активные задачи вкладчика

Задача считается активной, если:
- Статус = `IssueStatus.IN_PROGRESS`
- Вкладчик является создателем задачи (`creator_hash` содержит хеш вкладчика)
...
Когда вкладчик делает коммит в проект, можно зафиксировать часть незакоммиченного времени, но **только по завершённым задачам** (статус DONE):
...
### Статистика по проекту для вкладчика
...
### Проекты вкладчика со статистикой
...
1. **getTimeStats(data)** - Статистика времени вкладчика по проекту
2. **getContributorProjectsTimeStats(data)** - Проекты вкладчика со статистикой
...
- Каждый час система проверяет активных вкладчиков
- Для каждого вкладчика начисляет время за активные задачи
- Время распределяется равномерно между задачами

### 2. Просмотр статистики
- Вкладчик может посмотреть время по проектам
...
- Группировка по проектам/вкладчикам
...
1. **Индивидуальные лимиты**: Каждый вкладчик имеет индивидуальный лимит часов в день (поле `hours_per_day`), по умолчанию 8 часов (применяется только в периодическом распределении времени)
...
- Отслеживание ошибок обработки отдельных вкладчиков
...
Возможные улучшения:
- Настраиваемые лимиты времени на вкладчика
