...
2. **Равномерное распределение**: Если у вкладчика несколько активных задач, общее доступное время (8 часов в день) распределяется между ними поровну
3. **Лимиты**: Максимум 8 часов в день на вкладчика, независимо от количества задач
...
private async calculateTimeDistributionPerIssue(contributor, activeIssues, date) {
  const HOURS_PER_DAY = 8;        // Максимум часов в день
  ...
  // Если уже отработано 8 часов - выходим
...
1. **Лимиты**: Строго 8 часов в день на вкладчика (применяется только в периодическом распределении времени)
