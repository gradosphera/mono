 ...
 */
@Injectable()
export class ProgramInvestDeltaMapper
  implements IBlockchainDeltaMapper<IProgramInvestBlockchainData, ProgramInvestDomainEntity>
{
  ...
  extractEntityId(delta: IDelta): string {
    // В таблице progrinvests primary_key является ID программной инвестиции
  ...
  isRelevantDelta(delta: IDelta): boolean {
    ...
    const isRelevantTable =
      delta.table === 'progrinvests' || delta.table === 'progrinvests*' || delta.table.includes('progrinvests');
  ...
  private validateBlockchainData(data: any): boolean {
    ...
    const requiredFields = ['id', 'coopname', 'username', 'invest_hash', 'status', 'invested_at', 'statement', 'amount'];
  ...
  getSupportedTableNames(): string[] {
    return ['progrinvests', 'progrinvests*'];
  }
