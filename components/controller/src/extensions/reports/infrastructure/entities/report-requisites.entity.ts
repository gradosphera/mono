 ...
 *
 * Хранятся поля, которых нет в `IOrganizationData` (ончейн-данные): ОКВЭД,
 * ОКФС, ОКОПФ, ОКТМО, ОКПО, регномер СФР, должность руководителя, СНИЛС
 * подписанта, описание доверенности. Плюс возможные override'ы телефона
 * и адреса (если в отчётности нужен не тот, что в блокчейне).
 *
