...
export default async ({ page, context, shot, env }) => {
  ...
  const toggleShot = await shot(
    ...
    'Карточка компонента «MVP v1»: переключатель «Принимает инвестиции» в левой панели включён — пайщики могут вкладывать паевые взносы напрямую в этот компонент.',
  );
  ...
  const profileShot = await shot(
    ...
    'Страница профиля пайщика: кнопка «Инвестировать» на Кошельке Благороста — этот путь вкладывает паевой взнос в программу «Капитализация», совет распределяет его между компонентами.',
  );
