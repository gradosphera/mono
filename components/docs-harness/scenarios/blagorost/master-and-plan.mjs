...
// в прозе.
//
// Важно для следующего шага «Запуск компонента в работу»: запустить
// компонент можно только когда у него уже назначен мастер и
// родительский проект уже в работе. Без мастера коммиты некому
// принимать; пока не запущен родительский проект — компонент в работу
// тоже не пойдёт. Назначение мастера здесь — обязательный шаг перед
// запуском компонента.
//
// Кадры:
