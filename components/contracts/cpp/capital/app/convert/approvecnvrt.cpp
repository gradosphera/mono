void capital::approvecnvrt(eosio::name coopname, eosio::name application, eosio::name approver, checksum256 convert_hash, document2 approved_statement){
  ...
  eosio::check(exist_convert.has_value(), "Конвертация пользователя для проекта не найдена");
  ...
  auto contributor = Capital::Contributors::get_active_contributor_with_appendix_or_fail(coopname, convert -> project_hash, convert -> username);
  ...
  auto exist = Capital::Projects::get_project(coopname, convert -> project_hash);
  ...
  std::string memo = Capital::Memo::get_approve_convert_memo(contributor -> id, convert -> id);
