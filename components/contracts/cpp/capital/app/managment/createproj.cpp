 ...
 * Создаёт проект.
 *
 * Выполняет проверки отсутствия проекта с указанным хэшем и валидирует
 * родительский хэш согласно правилам: допускается либо пустой `parent_hash`,
 * либо ссылка на проект первого уровня без собственного родителя.
 * После проверок создаёт запись в таблице `projects` со статусом `created`.
 *
