void capital::setplan(name coopname, checksum256 project_hash, uint64_t plan_creators_time, asset plan_expenses, asset plan_hour_cost){
  ...
  eosio::check(plan_creators_time > 0, "План времени должен быть положительным");
  ...
  auto calculated_plan = Capital::Core::calculate_plan_generation_amounts(plan_hour_cost, plan_creators_time, plan_expenses);
