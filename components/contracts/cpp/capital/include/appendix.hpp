...
/**
 * @brief Класс-помощник для работы с приложениями к договорам УХД
 */
class AppendixHelper {
public:
    
    /**
     * @brief Получает приложение по хэшу
     */
    static std::optional<appendix> get_appendix(eosio::name coopname, const checksum256 &appendix_hash, eosio::name contract_name) {
        appendix_index appendixes(contract_name, coopname.value);
