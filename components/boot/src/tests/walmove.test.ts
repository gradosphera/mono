 ...
 * одного бух.счёта (без Dr/Cr). Кейс: после регистрации пайщика на 2002
 * (MIN_SHARE_FUND) и 2001 (SHARE_FUND_PAY) лежит вместе X RUB на 80-м счёте,
 * нам нужно перенести часть с 2002 на 2001 (например, реструктуризация
 * минимального паевого взноса в обычный после criteria-аудита).
 *
 * Проверяет:
 *  - mutation walmoveWallets отрабатывает успешно
 *  - balance кошельков 2002 (источник) и 2001 (получатель) изменился на amount
 *  - account 80 (Cr SHARE_FUND) ≡ не изменился (WALLET_ONLY не трогает счета)
...
describe('walmove (operation o.adj.walmove) — корректировка между кошельками одного бух.счёта', () => {
  it('переводит сумму между двумя кошельками счёта 80 без изменения balance счёта', async () => {
    // 1. Регистрируем нового пайщика — пополняет 2002 (MIN_SHARE_FUND) и 3001 (ENTRANCE_FEES)
    ...
    // 3. Перевод 2002 → 2001 (оба кошелька на счёте 80)
    ...
    // Главное свойство WALLET_ONLY: counterbalance бух.счёта не меняется.
  ...
  it('отказывает в переводе между кошельками разных бух.счетов (валидация в backend)', async () => {
    // 2002 (MIN_SHARE_FUND, на счёте 80) → 3001 (ENTRANCE_FEES, на счёте 86) — разные счета.
    let errMessage = ''
    try {
      await walmove(
        login, COOP, COOP,
