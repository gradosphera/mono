...
3. **В конце:** **`| <username> <hash>`** — **username** из `~/.claude/config/blago/config.yaml` или активной сессии blago; **hash** — полное поле **`hash`** из YAML того же `issues/…md`.

```text
[CAPITAL-12] краткое описание изменения | ant <полный-hash>
краткое описание | ant <полный-hash>
```
...
- Краткое описание.
- В конце **`| <username> <hash>`** — **username** из blago config/сессии; **hash** — полное поле **`hash`** из того же issue.

```text
[CAPITAL-42] правка API оплат | ant 0a1b2c3d4e5f…
фикс валидации | ant 0a1b2c3d4e5f…
```
