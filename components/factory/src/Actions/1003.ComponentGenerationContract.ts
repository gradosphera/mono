  ...
  async generateDocument(data: ComponentGenerationContract.Action, options?: IGenerationOptions): Promise<IGeneratedDocument> {
    ...
    const { template, coop, vars, userData, generatorAgreementUdata, generatorAgreementCreatedAtUdata, contributorContractUdata, contributorContractCreatedAtUdata } = await this.resolveParallel({
      template: () => process.env.SOURCE === 'local'
        ? Promise.resolve(ComponentGenerationContract.Template as ITemplate<ComponentGenerationContract.Model>)
        : this.getTemplate<ComponentGenerationContract.Model>(DraftContract.contractName.production, ComponentGenerationContract.registry_id, data.block_num),
