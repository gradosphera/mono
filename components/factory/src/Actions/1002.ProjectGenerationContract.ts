...
export class Factory extends DocFactory<ProjectGenerationContract.Action> {
  ...
  async generateDocument(data: ProjectGenerationContract.Action, options?: IGenerationOptions): Promise<IGeneratedDocument> {
    let template: ITemplate<ProjectGenerationContract.Model>

    if (process.env.SOURCE === 'local') {
      template = ProjectGenerationContract.Template
    }
    else {
      template = await this.getTemplate(DraftContract.contractName.production, ProjectGenerationContract.registry_id, data.block_num)
    }
