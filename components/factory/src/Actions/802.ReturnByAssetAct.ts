...
export class Factory extends DocFactory<ReturnByAssetAct.Action> {
  ...
  async generateDocument(data: ReturnByAssetAct.Action, options?: IGenerationOptions): Promise<IGeneratedDocument> {
    let template: ITemplate<ReturnByAssetAct.Model>

    if (process.env.SOURCE === 'local') {
      template = ReturnByAssetAct.Template
    }
    else {
      template = await this.getTemplate(DraftContract.contractName.production, ReturnByAssetAct.registry_id, data.block_num)
    }
