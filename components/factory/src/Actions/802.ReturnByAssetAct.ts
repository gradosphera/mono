...
import { ReturnByAssetAct } from '../Templates'
import { DocFactory } from '../Factory'
import type { IGeneratedDocument, IGenerationOptions, IMetaDocument, ITemplate } from '../Interfaces'
import type { MongoDBConnector } from '../Services/Databazor'
import type { ExternalOrganizationData } from '../Models'

export { ReturnByAssetAct as Template } from '../Templates'

export class Factory extends DocFactory<ReturnByAssetAct.Action> {
  constructor(storage: MongoDBConnector) {
    super(storage)
  }

  async generateDocument(data: ReturnByAssetAct.Action, options?: IGenerationOptions): Promise<IGeneratedDocument> {
    const { template, coop, vars, user, request, transmitter } = await this.resolveParallel({
      template: () => process.env.SOURCE === 'local'
        ? Promise.resolve(ReturnByAssetAct.Template as ITemplate<ReturnByAssetAct.Model>)
        : this.getTemplate<ReturnByAssetAct.Model>(DraftContract.contractName.production, ReturnByAssetAct.registry_id, data.block_num),
      ...
      transmitter: () => this.getUser(data.transmitter, data.block_num),
    })
    ...
    const combinedData: ReturnByAssetAct.Model = {
      meta,
      coop,
      vars,
      user: commonUser,
