...
export const REGISTRATION_PROGRAMS_CONFIG: ICooperativeRegistrationPrograms[] = [
  {
    ...
    programs: [
      ...
      },
      {
        ...
        applicable_account_types: [AccountType.individual, AccountType.entrepreneur],
        agreement_ids: ['capitalization_agreement'],
        order: 2,
      },
    ],
  },
...
getAgreementsForAccountType(
  ...
  const baseAgreements = REGISTRATION_AGREEMENTS_CONFIG.agreements
    ...
    .filter(a => a.id !== 'capitalization_agreement' && a.id !== 'generator_offer');
  ...
  if (coopname === 'voskhod' && accountType === AccountType.individual) {
    const capitalization = this.getAgreementById('capitalization_agreement');
    if (capitalization) {
...
- Для voskhod: если не передан `program_key`, используется старая логика (capitalization_agreement для физиков)
