...
AuthCard(:maxWidth="1000")
  div(v-if="token")
    ...
    .reset-key-header
      .text-h6.reset-key-title СОХРАНИТЕ КЛЮЧ
  ...
  div(v-else)
    //- Заголовок проверки почты
    .reset-key-header
      .text-h6.reset-key-title ПРОВЕРЬТЕ ПОЧТУ
      .subtitle.text-body2.text-grey-7.q-mt-sm
        | На вашу электронную почту отправлено письмо со ссылкой для перевыпуска ключа. Пожалуйста, перейдите по ссылке для продолжения.
