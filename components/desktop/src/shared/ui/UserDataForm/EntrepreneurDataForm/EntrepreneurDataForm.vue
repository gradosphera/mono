<template lang="pug">
div(v-if="userData.entrepreneur_data").q-gutter-sm.q-mt-md
  q-input(ref="firstInput" autofocus v-model="userData.entrepreneur_data.last_name" standout="bg-teal text-white" label="Фамилия" :rules="[val => notEmpty(val), val => validatePersonalName(val)]" autocomplete="off")
  q-input(v-model="userData.entrepreneur_data.first_name" standout="bg-teal text-white" label="Имя" :rules="[val => notEmpty(val), val => validatePersonalName(val)]" autocomplete="off")
  q-input(v-model="userData.entrepreneur_data.middle_name" standout="bg-teal text-white" label="Отчество" :rules="[val => validatePersonalName(val)]" autocomplete="off")
  ...
  q-input(v-model="userData.entrepreneur_data.phone" standout="bg-teal text-white" mask="+7 (###) ###-##-##" fill-mask label="Номер телефона" :rules="[val => notEmpty(val), val => notEmptyPhone(val)]" autocomplete="off")
  ...
  q-input(v-model="userData.entrepreneur_data.city" standout="bg-teal text-white" label="Город" :rules="[val => notEmpty(val)]" autocomplete="off")
  q-input(v-model="userData.entrepreneur_data.full_address" standout="bg-teal text-white" label="Адрес регистрации (как в паспорте)" :rules="[val => notEmpty(val)]" autocomplete="off")
