...
.user-data-stack(v-if="userData.entrepreneur_data")
  q-input(ref="firstInput" autofocus v-model="userData.entrepreneur_data.last_name" outlined color="primary" label="Фамилия" :rules="[val => notEmpty(val), val => validatePersonalName(val)]" autocomplete="off")
  q-input(v-model="userData.entrepreneur_data.first_name" outlined color="primary" label="Имя" :rules="[val => notEmpty(val), val => validatePersonalName(val)]" autocomplete="off")
  q-input(v-model="userData.entrepreneur_data.middle_name" outlined color="primary" label="Отчество" :rules="[val => validatePersonalName(val)]" autocomplete="off")
  ...
  q-input(v-model="userData.entrepreneur_data.phone" outlined color="primary" mask="+7 (###) ###-##-##" fill-mask label="Номер телефона" :rules="[val => notEmpty(val), val => notEmptyPhone(val)]" autocomplete="off")

  q-select(v-model="userData.entrepreneur_data.country" outlined color="primary" map-options emit-value option-label="label" option-value="value" label="Страна" :options="[{ label: 'Россия', value: 'Russia' }]" :rules="[val => notEmpty(val)]" autocomplete="off")

  q-input(v-model="userData.entrepreneur_data.city" outlined color="primary" label="Город" :rules="[val => notEmpty(val)]" autocomplete="off")
  q-input(v-model="userData.entrepreneur_data.full_address" outlined color="primary" label="Адрес регистрации (как в паспорте)" :rules="[val => notEmpty(val)]" autocomplete="off")
