...
export const context = `<div class="digital-document"><div style="text-align: right"><p>{% trans 'appendix_number' %} {{ result_act_short_hash }}</p><p>{% trans 'appendix_to_agreement' %} № {{ contributor_contract_number }}</p></div><div style="text-align: center"><h2>{% trans 'act_title' %} № АППИ-{{ result_act_short_hash }}</h2></div><div style="display: flex; justify-content: space-between;"><p>{% trans 'city_label' %} {{ coop.city }}</p><p>{% trans 'date_label' %} {{ meta.created_at }}</p></div><p>{{ vars.full_abbr }} "{{ vars.name }}" {% trans 'hereinafter_society' %} {% trans 'in_face_of_chairman' %} {{ coop.chairman.last_name }} {{ coop.chairman.first_name }} {{ coop.chairman.middle_name }}, {% trans 'and_shareholder' %} {{ common_user.full_name_or_short_name }}, {% trans 'act_text' %} № {{ contributor_contract_number }} {% trans 'from_date' %} {{ contributor_contract_created_at }} {% trans 'and_appendix' %} №{{ blagorost_agreement_number }} {% trans 'from_date' %} {{ blagorost_agreement_created_at }} {% trans 'blagorost_program_text' %} {% trans 'and_protocol' %} № {{ decision.id }} {% trans 'from_date' %} {{ decision.date }} {% trans 'following_property' %}:</p><table border="1" style="width: 100%; border-collapse: collapse; margin-top: 20px;"><tr><td style="font-weight: bold;">{% trans 'number_pp' %}</td><td style="font-weight: bold;">{% trans 'name_details' %}</td><td style="font-weight: bold;">{% trans 'property_form' %}</td><td style="font-weight: bold; width: 80px;">{% trans 'unit' %}</td><td style="font-weight: bold; width: 100px;">{% trans 'quantity' %}</td><td style="font-weight: bold; width: 120px;">{% trans 'total_cost' %}</td></tr><tr><td>1</td><td>{% trans 'copyright_object_description' %} {{ percent_of_result }}% {% trans 'in_copyright_object_short' %}, {% trans 'namely' %} №{{ result_hash }}.</td><td>{% trans 'copyright_right_description', percent_of_result, contributor_contract_number %}</td><td>{% trans 'share' %}</td><td>{{ percent_of_result }}%</td><td>{{ total_amount }}</td></tr><tr><td></td><td style="font-weight: bold;">{% trans 'total' %}</td><td></td><td></td><td>{{ percent_of_result }}%</td><td>{{ total_amount }}</td></tr></table><p>{% trans 'no_claims_quality' %}.</p><div style="margin-top: 40px;"><p><strong>{% trans 'transferred' %}:</strong></p><p>{% trans 'shareholder' %}: {{ common_user.full_name_or_short_name }}</p><p>{% trans 'signed_by_digital_signature' %}</p></div><div style="margin-top: 40px;"><p><strong>{% trans 'received' %}:</strong></p><p>{% trans 'chairman_title' %}</p><p>{{ vars.short_abbr }} "{{ vars.name }}"</p><p>{{ coop.chairman.last_name }} {{ coop.chairman.first_name }} {{ coop.chairman.middle_name }}</p><p>{% trans 'signed_by_digital_signature' %}</p></div></div><style>.digital-document {padding: 20px;white-space: pre-wrap;}</style>`
