...
- `cut-release.sh` — строго на `dev`. Версия `vYYYY.M.D` (повторный релиз того же
  дня → суффикс `-N`), `lerna version` (бамп всех package.json + lerna.json, коммит
  `chore(release): publish`, тег, push `dev`). **Деплой не запускает** — CI не
  слушает `dev`.
